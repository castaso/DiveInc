...
const model = 'transaction_liveaboard'
const table = 'transaction_liveaboards'
...
const mainModel = require('@models').transaction_liveaboard
const liveaboard = require('@models').liveaboard
const mainStatus = require('@models').transaction_liveaboard_status
  ...
  if(packageData.room){
    ...
    for(var i = 0; i < packageData.room.length; i++){
      ...
      for(var j = 0; j < packageData.room[i].roomItem.length; j++){
            ...
            var summaryPrice = 0
            // console.log(packageData.room[i].sharing_room)
            // if(packageData.room[i].sharing_room == "yes"){
            //     $(`#night${packageData.room[i].roomItem[j].id}`).text(packageData.room[i].roomItem[j].guest)
            //     summaryPrice = parseFloat(packageData.room[i].roomItem[j].guest) * parseFloat(packageData.room[i].roomPrice)
            // }else{
            //     $(`#night${packageData.room[i].roomItem[j].id}`).text("1")
            //     summaryPrice = 1 * parseFloat(packageData.room[i].roomPrice)
            // }
            ...
            //                 <div class="list-price">IDR&nbsp;${formatRupiah(parseFloat(packageData.room[i].roomItem[j].service[x].price) * parseFloat(packageData.room[i].roomItem[j].service[x].qty))}</div>
            //             </div>
            //         `)
...
controller.put(`/schedule`, middleware.checkToken, async (req,res) => {
  ...
  paramData.transaction_liveaboard_status_id = 'd2f95bbe-159b-4447-9e07-33925e04c5e9'
...
controller.put(`/extra`, middleware.checkToken, async (req,res) => {
  ...
  var paramData = {}
  paramData.package_data = package_data
  paramData.transaction_liveaboard_status_id = '59cfbb65-efff-4d25-aae1-b439efa62188'
...
controller.put(`/guest`, middleware.checkToken, async (req,res) => {
  ...
  var paramData = {}
  paramData.package_data = package_data
  paramData.transaction_liveaboard_status_id = 'b648ff73-468f-45cb-bbbc-fd3659e4b8ac'
  ...
  // console.log('Xxxxxxxxxxxxxx ============ EMAIL LIVEABOARD ============ xxxxxxxxxxxxxxX')
  // console.log(JSON.stringify(checkUpdate))

  let sendEmail = await template.emailLiveaboardRequestForUser(checkUpdate.dataValues)
  mailService(sendEmail)

  let sendEmail2 = await template.emailLiveaboardRequestForVendor(checkUpdate.dataValues, checkUpdate.dataValues.liveaboard.dataValues.user.dataValues.data_register.data.email)
...
controller.put(`/approve`, middleware.checkToken, async (req,res) => {
  ...
  getData.dataValues.type = "Liveaboard"
  // Panggil Xendit
  console.log("++++++++++++++++++++++++++++++++++++++++++++++++++++")
  //Fungsi XENDIT
  var createInvoice = await xendit.invoice(getData.dataValues)
  console.log("======================================================")
  console.log(createInvoice)

  var paramData = {}
  paramData.transaction_liveaboard_status_id = 'bbb8ff73-468f-45cb-babc-fd3444e4b8ac'
  paramData.total_price = loadTotal(getData.dataValues)
  paramData.diveinc_fee = {
    percent : generalData.liveaboard_fee,
    task : 10,
  ...
  let sendEmail = await template.emailLiveaboardApprove(checkUpdate.dataValues)
  mailService(sendEmail)
...
controller.get(`/list-checkout-room`, async (req,res) => {
  const getDataTransaction = await mainModel.findOne({
    where : {
      id : {[Op.eq] : req.params.id}
    },
    include : [
  ...
  if(!getDataTransaction) return res.status(404).send({success : false, message :"no data transaction found"})

  // CHCEK AVAILABLE 
  // const getDataTransactionGlobal = await mainModel.findAll({
  //   where : {
  //     transaction_liveaboard_status_id : 
  //   },
  //   include : [
  //     {
  //       model : mainStatus
  ...
  const dataRoom = await room.findAll({
    where: {
      parent_id: {[Op.eq] : getDataTransaction.dataValues.liveaboard_id}
    }
  })

  if(!dataRoom) return res.status(404).send({success : false, message :"no data room found"})

  // var packageData
  // for(var i = 0; i < dataRoom.length; i++){
  //   //roomAvailable = await r
  // }

  res.status(200).send({
    success : true,
    message : `List room ${model} successfully`,
    data: dataRoom
  })
});
...
controller.put(`/cancel`, middleware.checkToken, async (req,res) => {
  ...
  var paramData = {}
  // paramData.package_data = package_data
  paramData.transaction_liveaboard_status_id = 'baa8ff37-468f-455b-b5c5-fc34aae4b8ac'
