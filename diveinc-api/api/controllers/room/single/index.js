...
const mainModel = require('@models').room
...
controller.put(`/`, middleware.checkToken, async (req,res) => {
  ...
  if(!info || !price || !photos || !service || !parent_id || !rules) return res.status(403).send({success: false, message: "Invalid body"})
...
controller.get('/liveaboard', async (req,res) => {
