...
const mainModel = require('@models').resort
...
controller.post(`/`, middleware.checkToken, async (req,res) => {
  ...
  if(!profile || !highlight || !photos || !amenities || !how_to) return res.status(403).send({success: false, message: "Invalid body"})
...
controller.get(`/my-resort`, middleware.checkToken, async (req,res) => {
