...
const model = 'liveaboard'
const table = 'liveaboards'
...
controller.post(`/`, middleware.checkToken, async (req,res) => {
  ...
  if(!profile || !highlight || !photos || !amenities) return res.status(403).send({success: false, message: "Invalid body"})
...
controller.get(`/my-liveaboard`, middleware.checkToken, async (req,res) => {
