...
controller.post(`/`, async (req,res) => {
  ...
  if(!name || !description || !introduction 
    || !image_background || !image_icon) return res.send(403).send({success: false, message: "Invalid body"})

  let checkData = await mainModel.findOne({
      where : {
        name : {[Op.eq] : name},
        introduction : {[Op.eq] : introduction},
...
controller.get(`/datatable`, async (req,res) => {
  ...
  if(query.search.value != `` && query.search.value != null){
      paramQuery = `${paramQuery} AND (name ILIKE '%${query.search.value}%' OR introduction ILIKE '%${query.search.value}%' OR description ILIKE '%${query.search.value}%')`
      param = {
          active : true,
          [Op.or] : [
              {name : { [Op.iLike] : `%${query.search.value}%`}},
              {introduction : { [Op.iLike] : `%${query.search.value}%`}},
  ...
  switch (query.order[0].column) {
      case '0':
          ...
          break;
      case '1':
          orderQuery = `ORDER BY introduction ${query.order[0].dir}, updated_at DESC`
          order.unshift([`introduction`, query.order[0].dir])
          break;
      case '2':
