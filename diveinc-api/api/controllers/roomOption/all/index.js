...
const model = 'room_option'
const table = 'room_options'
...
const mainModel = require('@models').room_option
...
controller.get(`/datatable`, async (req,res) => {
  ...
  if(sanitizeSqlInput(query.search.value) != `` && sanitizeSqlInput(query.search.value) != null){
      paramQuery = `${paramQuery} AND (name ILIKE '%${sanitizeSqlInput(query.search.value)}%')`
      param = {
          active : true,
          [Op.or] : [
